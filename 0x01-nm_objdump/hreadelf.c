...
/**
 * swap_msb - swaps least significant bytes with msb
 * @oct: pointer to the first octect
 * @size: size of type *oct points to
 */
void swap_msb(char *oct, size_t size)
{
	size_t a, z;

	for (a = 0, z = size - 1; a < z; ++a, --z)
	{
		oct[a] ^= oct[z];
		oct[z] ^= oct[a];
		oct[a] ^= oct[z];
 ...
 */
char get_info(hdrs_t *hdr, int i)
{
	char c;
...
/**
 * print_sym - prints symbols for ELF file
 * @hdr: struct containing ELF header information
 *
 * Return: 1 on success, 0 if symtab not found
 */
int print_sym(hdrs_t *hdr)
{
	int i;
	size_t j, size;
	char *str_table;
	...
	size = GET_SHDR(sh_size, i) / GET_SHDR(sh_entsize, i);
	for (j = 1; j < size; ++j)
	{
		if (IS_MSB)
		{
			swap_msb((char *) SET_YHDR(st_value, j),
					SYM_SIZE(st_value, i));
			swap_msb((char *) SET_YHDR(st_info, j),
					SYM_SIZE(st_info, i));
			swap_msb((char *) SET_YHDR(st_name, j),
					SYM_SIZE(st_name, i));
		}
...
void init_shdr(hdrs_t *hdr)
{
	...
	if (IS_MSB)
		for (i = 0; i < GET_EHDR(e_shnum); ++i)
		{
			swap_msb((char *) SET_SHDR(sh_type, i),
					SHDR_SIZE(sh_type, i));
			swap_msb((char *) SET_SHDR(sh_offset, i),
					SHDR_SIZE(sh_offset, i));
			swap_msb((char *) SET_SHDR(sh_size, i),
					SHDR_SIZE(sh_size, i));
			swap_msb((char *) SET_SHDR(sh_entsize, i),
 ...
 * Converts entries to msb if needed
 */
void init_ehdr(hdrs_t *hdr)
{
	hdr->Ehdr32 = (Elf32_Ehdr *) hdr->addr;
	hdr->Ehdr64 = (Elf64_Ehdr *) hdr->addr;
	if (IS_MSB)
	{
		swap_msb((char *) SET_EHDR(e_shoff), EHDR_SIZE(e_shoff));
		swap_msb((char *) SET_EHDR(e_shentsize),
				EHDR_SIZE(e_shentsize));
		swap_msb((char *) SET_EHDR(e_shnum), EHDR_SIZE(e_shnum));
	}
}
